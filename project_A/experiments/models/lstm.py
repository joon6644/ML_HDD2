...
def train_lstm_model(X_train, y_train, X_val=None, y_val=None, seed: int = 42,
    ...
    if has_val:
        # Keep validation sequence lazy or convert reasonably
    ...
    if is_cost_sensitive:
        ...
        print(f"Training LSTM (Cost-Sensitive sqrt pos_weight={scale_pos_weight:.2f}, "
              f"max_epochs={hp['epochs']}, patience={hp['patience']}) on device: {device}...")
    else:
        pos_weight_tensor = None
        print(f"Training LSTM (Unweighted, FocalLoss={use_focal_loss}, "
              f"max_epochs={hp['epochs']}, patience={hp['patience']}) on device: {device}...")

    input_dim = X_train_t.shape[-1] if hasattr(X_train_t, 'shape') else X_train_t.X_raw.shape[1]
    model = LSTMClass(input_dim=input_dim, hidden_dim=hp['hidden_dim'],
