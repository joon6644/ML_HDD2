...
STYLE_DIR = os.path.join(PROJECT_ROOT, "analysis", "lead_time_analysis")
...
COL_LINE = "#8a8a8a"
COL_ONTIME = "#2b5c8f"
# Nothing here is pure black: as in Figs. 2-4 the darkest ink belongs to the
# drawn history, not to the labels or the event marks that annotate it.
COL_FAIL = fs.INK
COL_END = fs.INK_TICK
COL_BOUND = "#2b5c8f"
...
# Segment thickness is the one weight not taken from figure_style: these bars
# are the drawing itself, not annotation over it. They are set so the bars print
# as thick as they did before the figure was re-authored at its placement width.
LW_HIST = 3.4
LW_ONTIME = 4.6


def _segment(ax, x0, x1, y, color, lw=LW_HIST, **kw):
    ax.plot([x0, x1], [y, y], color=color, lw=lw, solid_capstyle="butt",
            zorder=3, **kw)


def _span(ax, x0, x1, y, label, color=fs.INK):
    """Dimension-style bracket naming the category of a stretch of history."""
    yb = y + 0.20
    ax.plot([x0, x1], [yb, yb], color=color, lw=fs.LW_SPINE, zorder=3)
    for x in (x0, x1):
        ax.plot([x, x], [yb - 0.045, yb], color=color, lw=fs.LW_SPINE, zorder=3)
    ax.text((x0 + x1) / 2, yb + 0.035, label, ha="center", va="bottom",
            fontsize=fs.FS_TITLE, fontweight="bold", color=color)


def _cond(ax, x0, x1, y, text, color=fs.INK_TICK):
    ax.text((x0 + x1) / 2, y - 0.13, text, ha="center", va="top",
            fontsize=fs.FS_TICK, color=color)
...
def main():
    ...
    fig, ax = plt.subplots(figsize=(fs.FULL_W, 2.95), dpi=fs.DPI)

    # ---- shared reference: the end of the observation window -----------------
    ax.plot([T_OBS_END, T_OBS_END], [Y_CENSORED - 0.42, Y_FAILED + 0.46],
            color=COL_END, ls="--", lw=fs.LW_RULE, zorder=4)
    ax.text(T_OBS_END, Y_FAILED + 0.50, r"Observation end ($t_{\mathrm{end},d}$)", ha="center",
            va="bottom", fontsize=fs.FS_TITLE, fontweight="bold", color=COL_END)

    # ---- row 1: failure observed inside the window --------------------------
    _segment(ax, T_START, T_H_FAILED, Y_FAILED, COL_LINE)
    _segment(ax, T_H_FAILED, T_FAIL, Y_FAILED, COL_ONTIME, lw=LW_ONTIME)
    _span(ax, T_START, T_H_FAILED, Y_FAILED, "Early")
    _span(ax, T_H_FAILED, T_FAIL, Y_FAILED, "On-time", color=COL_ONTIME)
    _cond(ax, T_START, T_H_FAILED, Y_FAILED, r"$\mathrm{LT}_d > H$")
    _cond(ax, T_H_FAILED, T_FAIL, Y_FAILED, r"$\mathrm{LT}_d \leq H$", color=COL_ONTIME)
    _tick(ax, T_H_FAILED, Y_FAILED, COL_BOUND, ":", fs.LW_RULE)
    # The failure mark keeps its prominence through height, not through weight:
    # every reference rule in the paper's figures is drawn at LW_RULE.
    _tick(ax, T_FAIL, Y_FAILED, COL_FAIL, "-", fs.LW_RULE, half=0.15)
    ax.text(T_FAIL, Y_FAILED - 0.30, r"$t_{\mathrm{failure},d}$", ha="center",
            va="top", fontsize=fs.FS_TICK, fontweight="bold", color=COL_FAIL)
    ax.text(T_H_FAILED, Y_FAILED - 0.30, r"$t_{\mathrm{failure},d} - H$",
            ha="center", va="top", fontsize=fs.FS_TICK, color=COL_BOUND)

    # ---- row 2: right-censored ---------------------------------------------
    _segment(ax, T_START, T_H_CENSORED, Y_CENSORED, COL_LINE)
    _segment(ax, T_H_CENSORED, T_OBS_END, Y_CENSORED, COL_EXCLUDE)
    ...
    _cond(ax, T_START, T_H_CENSORED, Y_CENSORED, r"$\mathrm{LT}_d > H$ guaranteed")
    _cond(ax, T_H_CENSORED, T_OBS_END, Y_CENSORED, "Excluded", color=COL_LINE)
    _tick(ax, T_H_CENSORED, Y_CENSORED, COL_BOUND, ":", fs.LW_RULE)
    ...
    for y, row_name, no_alarm in (
        ...
        ax.text(T_START - 14, y, row_name, ha="right", va="center",
                fontsize=fs.FS_TITLE, fontweight="bold", color=fs.INK)
        ax.text(X_COL_C, y, no_alarm, ha="center", va="center",
                fontsize=fs.FS_TITLE, fontweight="bold", color=fs.INK)

    ax.annotate("", xy=(T_OBS_END + 6, Y_CENSORED - 0.55),
                xytext=(T_START, Y_CENSORED - 0.55),
                arrowprops=dict(arrowstyle="->", color=fs.GRID, lw=fs.LW_SPINE))
    ax.text(T_OBS_END + 6, Y_CENSORED - 0.62, "Time ($t$)", ha="right", va="top",
            fontsize=fs.FS_TICK, color=fs.GRID)

    handles = [
        Line2D([], [], color=COL_ONTIME, lw=LW_ONTIME,
               label=f"On-time window (within $H$ days before failure, $H={H_DAYS}$)"),
        Line2D([], [], color=COL_LINE, lw=LW_HIST, label="Evaluated observation history"),
        Line2D([], [], color=COL_EXCLUDE, lw=LW_HIST, label="Excluded interval"),
    ]
    ...
    ax.legend(handles=handles, loc="upper center",
              ...
              ncol=3, frameon=False, fontsize=fs.FS_NOTE)

    ax.set_xlim(X_LIM_L, X_LIM_R)
    ax.set_ylim(Y_CENSORED - 0.84, Y_FAILED + 0.70)
    ax.set_xticks([])
    ax.set_yticks([])
