...
BAR_ALPHA, BAR_EDGE = fs.BAR_ALPHA, fs.BAR_EDGE
MAX_DAYS = 360
...
def main():
    ...
    fig, axes = plt.subplots(2, 1, figsize=(fs.COL_W, 3.7), dpi=fs.DPI,
                             sharex=True, sharey=True,
                             gridspec_kw={"hspace": 0.18})
    # No suptitle, as in the other two figures: the Korean caption below names
    # the figure and gives the dataset, model and seed, so a title here is the
    # same text twice and costs two lines of column height.
    ...
    for idx, (code, title) in enumerate(MODELS):
        ...
        ax.bar(edges[:-1], counts, width=np.diff(edges), align="edge",
               ...
               linewidth=fs.LW_EDGE, zorder=3)
        ax.bar(366, n_over, width=18, align="edge", facecolor=STYLE[code],
               alpha=0.45, edgecolor=BAR_EDGE, linewidth=fs.LW_EDGE, hatch="//", zorder=3)
        if n_over:
            ax.text(375, n_over + 0.4, str(n_over), ha="center",
                    fontsize=fs.FS_NOTE, color=fs.INK)

        med = float(np.median(days))
        ax.axvline(med, color="#d9534f", linestyle="--", linewidth=fs.LW_RULE, zorder=5)
        ax.text(med + 8, 11.4, f"median {med:.0f} d", fontsize=fs.FS_NOTE,
                color="#c0392b", fontweight="bold", va="top")

        ax.set_title(f"{labels[idx]} {title}  (n = {n}, On-time = {n_on})",
                     fontsize=fs.FS_TITLE, fontweight="bold", loc="left", pad=fs.TITLE_PAD)
        ax.set_ylim(0, 12.2)
    ...
    ax.set_xticklabels([str(v) for v in np.arange(0, 301, 60)] + [">360"], fontsize=fs.FS_TICK)
    ax.set_xlabel("Lead time (days)", fontsize=fs.FS_LABEL, fontweight="bold", labelpad=fs.LABELPAD)
    fig.supylabel("Number of HDDs", fontsize=fs.FS_LABEL, fontweight="bold",
