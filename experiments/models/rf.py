...
def train_rf_model(X_train, y_train, X_val=None, y_val=None,
    ...
    - RF does not support iterative early stopping; instead oob_score is enabled
      as a proxy for generalisation error (comparable to val loss in other models).
    - Val set is used to report a real Val AUROC after training, alongside oob_score,
      for comparability with the val-loss-based reporting of the other five models.
    ...
    if is_cost_sensitive:
        ...
        print(f"Training Random Forest (Cost-Sensitive sqrt scale_pos_weight={scale_pos_weight:.2f}, "
              f"n_estimators={hp['n_estimators']}, oob_score=True)...")
    else:
        class_weight = None
        print(f"Training Random Forest (Unweighted, n_estimators={hp['n_estimators']}, oob_score=True)...")
    ...
    print(f"Random Forest training complete.  OOB Score (proxy val): {rf.oob_score_:.4f}{val_auc_str}")
    return rf
