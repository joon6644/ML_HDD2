...
def main():
    ...
    print(f" Early Stopping             : ENABLED across all models (Val Set)")
    ...
    print(f" Drop Failure Day in Train  : {config.DROP_FAILURE_DAY_IN_TRAIN}")
    if args.sample_size:
        print(f" ⚠️ FAST TEST MODE          : Sample size limited to {args.sample_size} disks")
    print("=" * 60)

    # 1. Load Data
    train_df, val_df, test_df, features = load_dataset(args.data)
    ...
    if is_sequence_model:
        ...
        if args.imbalance == 'easyensemble':
            ...
            X_train_proc, y_train_proc = apply_imbalance_treatment(X_train_seq, y_train_seq, strategy=args.imbalance, seed=args.seed, dataset_path=args.data)
            if args.model == 'lstm':
        ...
        if args.imbalance == 'easyensemble':
            ...
            for k, (X_sub, y_sub) in enumerate(subsets):
                ...
                if args.model == 'rf':
                    m = train_rf_model(X_sub, y_sub, seed=sub_seed, is_cost_sensitive=is_cost_sensitive)
                elif args.model == 'lgbm':
                    m = train_lgbm_model(X_sub, y_sub, X_val_2d, y_val, seed=sub_seed, is_cost_sensitive=is_cost_sensitive)
                elif args.model == 'xgb':
                    m = train_xgb_model(X_sub, y_sub, X_val_2d, y_val, seed=sub_seed, is_cost_sensitive=is_cost_sensitive)
                elif args.model == 'mlp':
            ...
            X_train_proc, y_train_proc = apply_imbalance_treatment(X_train_2d, y_train, strategy=args.imbalance, seed=args.seed, dataset_path=args.data)
            if args.model == 'rf':
