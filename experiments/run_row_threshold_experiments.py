...
def run_row_threshold_experiments():
    parser = argparse.ArgumentParser(description="Script 1: Row-Level Threshold Search & Evaluation (3 Datasets x 4 Models x 5 Seeds)")
    ...
    master_csv_path = os.path.join(project_dir, "results", "master_row_threshold_results.csv")
    ...
    print("=" * 80)
    print(" SCRIPT 1: ROW-LEVEL THRESHOLD SEARCH & DUAL EVALUATION RUNNER")
    ...
    print(f" Threshold Search    : Row-Level Val Set (FAR <= 1% Constraint -> Max Recall)")
    ...
    for ds in unique_datasets:
        ...
        for idx, t in enumerate(ds_tasks, 1):
            ...
            try:
                ...
                print(f"[Row Threshold Search] Best Threshold on Val Set: {th_row:.4f} (Max Val Row Recall @ FAR <= 1%: {max_val_rec:.4%})")

                # E. Evaluate Test Set using th_row
                # 1) Row-level evaluation on Test Set
                row_metrics = calculate_row_level_metrics(test_y_true, test_probs, threshold=th_row)

                # 2) Proposed Disk-level evaluation on Test Set using th_row
                proposed_metrics, _ = evaluator.evaluate_proposed_level(raw_preds_test, threshold=th_row)
                ...
                row_data = {
                    ...
                    # Proposed Disk-level Metrics (evaluated at th_row)
                    'Proposed Disk Precision': round(proposed_metrics['precision'], 4),
                    'Proposed Disk Recall': round(proposed_metrics['recall'], 4),
                    'Proposed Disk F1': round(proposed_metrics['f1'], 4),
                    'Proposed Disk FAR (%)': round(proposed_metrics['far'] * 100, 2),
                    'Proposed Median LT': round(proposed_metrics['median_lead_time'], 2),
                    'Proposed EDR@15': round(proposed_metrics['edr_15'], 4)
                }
