...
def plot_discrete_histogram(df_records: pd.DataFrame, hdd_name: str, model_name: str, total_failed: int, thresh: float, output_dir: str):
    ...
    # Discrete 1-day bins centered on integer days [0..30]
    ...
    # Annotate integer count on top of non-zero bars
    for count, edge in zip(counts, edges):
        if count > 0:
            day_center = edge + 0.5
            plt.text(
                day_center, count + max(counts) * 0.015,
                f"{int(count)}",
                ha="center", va="bottom",
                fontsize=9, fontweight="bold"
            )

    mean_lt = float(np.mean(lead_times_in_range)) if len(lead_times_in_range) > 0 else 0.0
    median_lt = float(np.median(lead_times_in_range)) if len(lead_times_in_range) > 0 else 0.0

    if len(lead_times_in_range) > 0:
        plt.axvline(median_lt, color="darkred", linestyle="--", linewidth=2.5, label=f"Median Lead Time: {median_lt:.1f} days")
        plt.axvline(mean_lt, color="darkorange", linestyle="-.", linewidth=2.5, label=f"Mean Lead Time: {mean_lt:.2f} days")
    ...
    stats_text = (
        ...
        f"Alarms (0-30d): {len(lead_times_in_range)}\n"
        f"Mean Lead Time: {mean_lt:.2f} days\n"
    ...
    plt.gca().text(
        ...
        bbox=dict(boxstyle="round,pad=0.5", facecolor="white", edgecolor="#bbbbbb", alpha=0.9)
    )

    plt.title(f"Lead Time Distribution (1-Day Unit) - {mfr_name} ({model_upper})", fontsize=14, fontweight="bold", pad=15)
    plt.xlabel("Lead Time (Days from First Alarm to Actual Failure)", fontsize=11, labelpad=10)
    plt.ylabel("Disk Count (First Alarms)", fontsize=11, labelpad=10)
    plt.xlim(-0.7, 30.7)
